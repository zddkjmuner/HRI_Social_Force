...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        for agent in self.agents:
            ...
            #u_action_space = u_action_space.append()
            if agent.movable:
    ...
    def step(self, action_n):
        ...
        for agent in self.agents:
            ...
            done_n.append(self._get_done(agent, self.world))
    ...
    def _get_done(self, agent, world):
        all_dist = []
        all_ang_dist = []
        flag = None
        # for l, landmark in enumerate(world.landmarks):
        #     if l < world.num_goals:
        #         dists_goal = np.sqrt(np.sum(np.square(agent.state.p_pos - landmark.state.p_pos)))
        #         ang_dists_goal = np.abs(agent.state.p_ang - landmark.state.p_ang) 
        #         if int(dists_goal)==0:
        #             flag = 1
        #         else:
        #             return False
        #         if int(ang_dists_goal)==0:
        #             flag = 1
        #         else:
        #             return False
        # if flag==1:
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        agent.action.c = np.zeros(self.world.dim_c)
        #agent.action.phi = np.zeros(self.world.dim_ang)
        # process action
        ...
        if agent.movable:
            ...
            if self.discrete_action_input:
                agent.action.u = np.zeros(self.world.dim_p)
                #agent.action.phi = np.zeros(self.world.dim_ang)
                # process discrete action for position
                if action[0] == 1: agent.action.u[0] = -1.0
                if action[0] == 2: agent.action.u[0] = +1.0
                if action[0] == 3: agent.action.u[1] = -1.0
                if action[0] == 4: agent.action.u[1] = +1.0
                # process discrete action for rotation
                if action[0] == 5: agent.action.phi[0] = -1.0
                if action[0] == 6: agent.action.phi[0] = +1.0    
            else:
                if self.force_discrete_action:
                    d = np.argmax(action[0])
                    action[0][:] = 0.0
                    action[0][d] = 1.0
                if self.discrete_action_space: # implement this situation
                    agent.action.u[0] += action[0][0] - action[0][1]
                    agent.action.u[1] += action[0][2] - action[0][3]
                    #agent.action.phi += action[0][4] - action[0][5]
                else:
